...
// ===== PLAYER DE VÍDEO =====
// Converte qualquer link do YouTube (watch, youtu.be, shorts, embed) em URL de embed
...
function buildPlayerInner(anime, episode) {
    const videoUrl = episode ? episode.video : '';
    const label = episode
        ? `${anime.title} - T${episode.season || 1} Ep. ${episode.number}${episode.title ? ': ' + episode.title : ''}`
        : anime.title;
    ...
    if (!videoUrl) {
        return `
            ...
            <p style="color: var(--text-secondary);">Nenhum vídeo cadastrado para este episódio ainda.</p>
        `;
    }
    ...
    if (youtubeEmbed) {
        return `
            <div style="width:100%; aspect-ratio:16/9;">
                <iframe src="${youtubeEmbed}" style="width:100%; height:100%; border:0; border-radius:5px;"
                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                        allowfullscreen></iframe>
            </div>
            <h3 style="color: #fff; margin-top: 0.8rem;">${label}</h3>
        `;
    }

    // Link direto de vídeo (mp4/webm) ou arquivo enviado (data:)
    return `
        <video src="${videoUrl}" controls autoplay style="width:100%; max-height:70vh; border-radius:5px; background:#000;"></video>
        <h3 style="color: #fff; margin-top: 0.8rem;">${label}</h3>
    `;
}
...
function playAnime(id, episodeId = null) {
    ...
    const episode = episodeId
        ? animeEpisodes.find(ep => ep.id === episodeId)
        : animeEpisodes[0];

    const newProgress = Math.min(100, (anime.progress || 0) + 5);
    updateProgress(id, newProgress);

    showNotification('🎬 Reproduzindo', episode
        ? `<strong>${anime.title}</strong> - Ep. ${episode.number}`
    ...
    const playerHTML = `
        <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #000; z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center;">
            <div style="position: relative; width: 90%; max-width: 900px; background: #1a1a1a; border-radius: 10px; padding: 20px;">
                ...
                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    ${buildPlayerInner(anime, episode)}
                </div>
            </div>
        </div>
    `;
