/**
 * bygdëall animes - Aplicação Principal
...
const CONFIG = {
    ...
    CACHE_KEY: 'bygdeall-animes_cache',
    MAX_ITEMS: 12,
...
function loadEpisodesData() {
    try {
        const saved = JSON.parse(localStorage.getItem('admin_episodes') || 'null');
        if (Array.isArray(saved)) return saved;
    } catch (e) {
        console.error('Erro ao carregar episódios do admin:', e);
    }
    return [];
}
...
// ===== PLAYER DE VÍDEO =====
//
// getVideoSource() é o ponto único que decide COMO tocar um link de vídeo.
// Pra adicionar um novo serviço no futuro (Bunny.net, Vimeo, Cloudflare Stream, etc.),
// basta adicionar um novo "if" aqui retornando { type: '...', embedUrl / directUrl }.
// O resto do player (buildPlayerInner) não precisa mudar.
function getVideoSource(url) {
    if (!url) return { type: 'none' };

    // --- Vídeo salvo neste aparelho (IndexedDB, sem servidor) ---
    if (url.startsWith('idb:')) {
        return { type: 'idb', ref: url };
    }
    ...
    for (const pattern of ytPatterns) {
        ...
        if (match) {
            ...
            return { type: 'youtube', embedUrl: `https://www.youtube-nocookie.com/embed/${match[1]}?${params.toString()}` };
        }
    }
    ...
    // --- Google Drive ---
    // Aceita o link normal que o botão "Compartilhar" do Drive gera, tipo:
    // https://drive.google.com/file/d/ARQUIVO_ID/view?usp=sharing
    const driveMatch = url.match(/drive\.google\.com\/file\/d\/([\w-]+)/);
    if (driveMatch) {
        return { type: 'drive', embedUrl: `https://drive.google.com/file/d/${driveMatch[1]}/preview` };
    }

    // --- TODO: Bunny.net Stream / Cloudflare Stream ---
    // Quando tiver conta em algum desses, é só adicionar aqui, por exemplo:
    // const bunnyMatch = url.match(/iframe\.mediadelivery\.net\/embed\/(\d+)\/([\w-]+)/);
    // if (bunnyMatch) return { type: 'bunny', embedUrl: url + '?autoplay=true' };

    // --- Link direto de vídeo (mp4/webm/ogg) ou arquivo enviado (data:) ---
...
function buildPlayerBody(videoUrl) {
    ...
    if (source.type === 'none') {
        return `
            <div class="player-empty">
                ...
                <p>Nenhum vídeo cadastrado para este episódio ainda.</p>
            </div>
        `;
    }

    if (source.type === 'direct') {
        return `<video src="${source.directUrl}" controls autoplay playsinline class="player-video"></video>`;
    }

    if (source.type === 'idb') {
        // O Blob mora no IndexedDB, então não dá pra montar o <video> na hora
        // (isso é síncrono). Mostra um placeholder e o mountLocalVideo() troca
        // pelo player de verdade assim que o Blob for lido.
        return `
            <div class="player-empty" id="idbVideoSlot" data-ref="${source.ref}">
                <i class="fas fa-spinner fa-spin"></i>
                <p>Carregando vídeo salvo neste aparelho...</p>
            </div>
        `;
    }

    // youtube, vimeo, ou qualquer outro serviço de embed futuro
    return `
        <iframe src="${source.embedUrl}" class="player-video" frameborder="0"
...
// ===== AÇÕES DOS ANIMES =====
function playAnime(id, episodeId = null) {
    const anime = getAnimeById(id);
    if (!anime) return;
    ...
    const animeEpisodes = getEpisodesForAnime(id);
    const currentIndex = episodeId
        ? animeEpisodes.findIndex(ep => ep.id === episodeId)
        : (animeEpisodes.length > 0 ? 0 : -1);
    const episode = currentIndex >= 0 ? animeEpisodes[currentIndex] : null;
    const prevEpisode = currentIndex > 0 ? animeEpisodes[currentIndex - 1] : null;
    const nextEpisode = currentIndex >= 0 && currentIndex < animeEpisodes.length - 1 ? animeEpisodes[currentIndex + 1] : null;

    const newProgress = Math.min(100, (anime.progress || 0) + 5);
    updateProgress(id, newProgress);

    const episodeLabel = episode
        ? `T${episode.season || 1} · Episódio ${episode.number}${episode.title ? ' — ' + episode.title : ''}`
        : 'Sem episódios cadastrados';
        ...
        <div class="video-player-overlay" id="videoPlayerOverlay">
            <div class="video-player-card">
                <div class="video-player-header">
                    <img src="${anime.image}" alt="${anime.title}" class="video-player-poster">
                    <div class="video-player-titles">
                        <h3>${anime.title}</h3>
                        <span>${episodeLabel}</span>
                    </div>
                    <button class="video-player-close" onclick="closeVideoPlayer()" aria-label="Fechar">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="video-player-body">
                    ${buildPlayerBody(episode ? episode.video : '')}
                </div>
                <div class="video-player-footer">
                    <button class="video-player-nav-btn" ${prevEpisode ? '' : 'disabled'}
                            onclick="${prevEpisode ? `playAnime(${id}, ${prevEpisode.id})` : ''}">
                        <i class="fas fa-step-backward"></i> Anterior
                    </button>
                    <span class="video-player-progress-label">${anime.progress || 0}% assistido</span>
                    <button class="video-player-nav-btn" ${nextEpisode ? '' : 'disabled'}
                            onclick="${nextEpisode ? `playAnime(${id}, ${nextEpisode.id})` : ''}">
    ...
    const idbSlot = document.getElementById('idbVideoSlot');
    if (idbSlot) mountLocalVideo(idbSlot.dataset.ref, idbSlot);
}

// Busca o Blob do vídeo no IndexedDB e troca o placeholder por um <video> de verdade.
// currentObjectUrl fica guardado pra ser liberado (revokeObjectURL) quando o player fechar,
// evitando vazamento de memória.
let currentObjectUrl = null;

async function mountLocalVideo(ref, slotEl) {
    try {
        const record = await VideoStore.getVideo(ref);
        if (!slotEl.isConnected) return; // usuário já fechou o player antes de terminar de carregar
        if (!record || !record.blob) {
            slotEl.innerHTML = `
                <i class="fas fa-video-slash"></i>
                <p>Este vídeo foi salvo em outro navegador/aparelho e não está disponível aqui.</p>
            `;
            return;
        }
        currentObjectUrl = URL.createObjectURL(record.blob);
        slotEl.outerHTML = `<video src="${currentObjectUrl}" controls autoplay playsinline class="player-video"></video>`;
    } catch (e) {
        console.error('Erro ao carregar vídeo local:', e);
        slotEl.innerHTML = `<i class="fas fa-video-slash"></i><p>Não foi possível carregar este vídeo.</p>`;
    }
}
...
function shareAnime(id) {
    ...
    if (navigator.share) {
        navigator.share({
            title: anime.title,
            text: `Assista ${anime.title} no bygdëall animes!`,
            url: window.location.href
        }).catch(() => {});
    } else {
        const shareText = `🎬 ${anime.title}\n📝 ${anime.description}\n⭐ ${anime.rating}/10\n\nAssista no bygdëall animes!`;
            ...
            <div class="modal-content" style="max-width: 800px;">
                ...
                <div style="margin-top: 2rem; border-top: 1px solid var(--border-color); padding-top: 1.5rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 0.5rem;">
                        ${getEpisodesForAnime(anime.id).length > 0 ? getEpisodesForAnime(anime.id).map(ep => `
                            <button class="btn btn-secondary" style="font-size: 0.8rem; padding: 0.5rem; justify-content: center;" 
                                    onclick="playAnime(${anime.id}, ${ep.id}); closeDetailsModal();">
                                <i class="fas fa-play-circle"></i> T${ep.season || 1} Ep. ${ep.number}
                            </button>
                        `).join('') : `
                            <p style="color: var(--text-secondary); font-size: 0.9rem;">Nenhum episódio cadastrado ainda no painel admin.</p>
                        `}
                    </div>
                </div>
            </div>
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    setTimeout(() => {
        showNotification('📱 Instale o App', 
            'Instale o bygdëall animes no seu dispositivo. <button onclick="installApp()" class="btn btn-primary btn-sm" style="margin-top: 0.5rem;">Instalar</button>'
        );
    }, 5000);
});
...
function init() {
    ...
    console.log('🚀 bygdëall animes carregado com sucesso!');
...
window.addEventListener('storage', (e) => {
    if (e.key === 'admin_animes' || e.key === 'admin_categories' || e.key === 'admin_episodes') {
        refreshFromAdmin();
    }
});
